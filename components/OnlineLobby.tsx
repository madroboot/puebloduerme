...
import { GameRoom, LobbyPlayer, Role } from '../types.ts';
import { ROLES_BY_PLAYER_COUNT } from '../constants.ts';
    ...
    <div className="w-full max-w-xl bg-gray-800 rounded-lg shadow-xl p-8">
      ...
      <div className="space-y-3 mb-8 h-48 overflow-y-auto pr-2">
        {Object.values(room.lobbyPlayers).map((player) => (
          <div key={player.uid} className="flex justify-between items-center bg-gray-700 p-3 rounded-lg">
            <span className={`font-semibold ${player.uid === userId ? 'text-green-400' : ''}`}>{player.name}</span>
